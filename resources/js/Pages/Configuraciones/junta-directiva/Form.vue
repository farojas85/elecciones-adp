<template>
    <form @submit.prevent="guardar">
        <div class="modal fade" id="modal-junta-directiva">
...
import useGradoMinisterial from '@/Composables/configuracion/junta-directivas'

export default {
    props:{
        form: Object
    },
    emits:['onJuntaDirectivas'],
    setup(props, { emit }) {
        ...
        const {
            errors, respuesta,
            agregarJuntaDirectiva, actualizarJuntaDirectiva
        } = useGradoMinisterial()

        const agregar = async() => {
            await agregarJuntaDirectiva(form.value)
            ...
            {
                $('#modal-junta-directiva').modal('hide')
                ...
                emit('onJuntaDirectivas')
            }
        }

        const actualizar = async() => {
             await actualizarJuntaDirectiva(form.value)
